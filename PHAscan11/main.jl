...
const hvX_eV_ = [40.8]; #collect(0:-0.2:-2)[1:end-1] .+ 42.8; # central photon energies of the gaussian pulses
const Tw = (-3, 5); # in fs, time window of simulation
const streak_wvl::Float64 = 1.85*sqrt(2); # in um, central wavelength of streaking field
const out_h5path::String = get_outdir(@__DIR__) * (STRK ? "/phAnz.h5" : "/phA0p0.h5"); # path to the output h5 file
const tauX::Float64 = 0.3*sqrt(2);
...
config["Kmax"] = 2.0 # in a.u.
config["Kmin"] = 0.2; # in a.u.
config["Ip"] = Ip1;
config["tauX"] = tauX;
