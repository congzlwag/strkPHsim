...
const hvX_eV_ = 40.8 .+ collect(-2.0:0.2:2.0); #  collect(-1.9:0.2:1.9); # central photon energies of the gaussian pulses
const Tw = (-3, 5); # in fs, time window of simulation
const streak_wvl::Float64 = 1.85; # in um, central wavelength of streaking field
const out_h5path::String = get_outdir(@__DIR__) * (STRK ? "/phAnz_sup7.h5" : "/phA0p0_sup7.h5"); # path to the output h5 file
...
# config["Kmax"] = STRK ? 3.2 : 1.7;  # in a.u.
# config["Kmin"] = 0.9; # 0.32; # in a.u.
config["Ip"] = Ip1;
config["tauX"] = tauX;
...
"""
    ...
    for (i,Amax) in enumerate(Amax_)
        ...
        print("INFO: A=$(Amax) Computing......")
        A_L = A1_L .* Amax;
        t0 = now()
        densities = PHInt_Pspace(E_X_reshaped, A_L, taxis, config);
        end_time = now()
        println("Done! Elapsed $(Dates.value(end_time - t0) / 1000) sec")
        print("    Reshape&Save......")
        t0 = now()
...
const Amax_full = collect(0.48:0.04:0.65)[2:end]
# const Amax_full = collect(0.64:0.06:0.9)[2:end]
